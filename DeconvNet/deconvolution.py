...
class Deconvolution:
    ...
    def __init__(self, conv_base_model=None):
        ...
        self.conv_sub_models = [None] + [AlexNet(i, self.conv_base_model) for i in (1, 2, 3, 4, 5)]  # Make it 1-based
                                                                   #highest_layer_num=None, base_model=None Don't no????????
    ...
    def project_down(self, image_path, layer, f=None, use_bias=False):      ## f means filter
        assert type(layer) == int                                        ## layer=5 there 
